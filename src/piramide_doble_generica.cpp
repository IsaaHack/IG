...
Piramide_doble_generica::Piramide_doble_generica(float radio, float alto, int num_lados){
    ...
    puntos[i] = Vector3D(radio * (1 + cos(2 * M_PI * i / num_lados)), 0, radio * (1 + sin(2 * M_PI * i / num_lados)));
    }
    puntos[this->num_lados + 1] = Vector3D(radio, alto, radio);
    puntos[this->num_lados + 2] = Vector3D(radio, -alto, radio);
}
